...
locals {
  ...
  runcmd_lustre_mount = var.create_lustre && local.lustre_mount_ip != "" ? format(
    "curl -sL -o /var/run/oke-lustre-mount.sh https://raw.githubusercontent.com/OguzPastirmaci/oci-hpc-oke/refs/heads/main/files/oke-lustre-mount.sh && (bash /var/run/oke-lustre-mount.sh '%v' '%v' '%v' || echo 'Error mounting Lustre' >&2)",
